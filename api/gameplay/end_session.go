...
func handleEndSession(conn *SafeWebSocket, sessionUUID string) {
	...
	// Delete the session cache from Redis
	err = Rdb.Del(ctx, sessionUUID).Err()
	if err != nil {
		log.Printf("Error deleting session cache from Redis: %v", err)
	} else {
		log.Printf("Session cache deleted successfully for session: %s", sessionUUID)
	}
